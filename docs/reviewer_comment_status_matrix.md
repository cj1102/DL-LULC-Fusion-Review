...
| 3. Missing fields, screening, cleaning, parameters, and 80.9% coding basis | Partial | WoSCC interface/field/default coverage, query, filters, export, screening, duplicate rule, keyword cleaning, affiliation-name check, VOSviewer version and retained threshold evidence are now reported. The manuscript now explains that all 261 records underwent a second-stage content screen and that 163 satisfied the explicit fusion-level eligibility rule; a record-level supplement documents those assignments. The eligible-subset percentage has been removed from the Abstract and Conclusion so that it is not mistaken for a 261-record distribution. | The exact frozen 261-record UT corpus and several original VOSviewer GUI/project settings cannot be recovered from retained files. The surviving workbook preserves the 163 included assignments but not a complete 261-row eligibility log identifying all 98 screened-out records. A fully closed record-level reproducibility claim would require the frozen corpus and exclusion crosswalk. |
