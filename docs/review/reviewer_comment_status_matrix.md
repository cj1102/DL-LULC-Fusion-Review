...
| 2. Expand bibliometric methods and add a flow diagram | Addressed | *Data Collection* and *Research Framework and Methodology* report WoSCC Advanced Search, Topic (`TS`), default subscription index coverage, full Boolean query, period, language, document types, export format, screening counts, duplicate rule, keyword thesauri, and VOSviewer 1.6.20. The workflow reports 283 to 281 to 261. A public tagged-text file and UT/title manifest identify the same 261 analytical records. Two cropped panels document the visible VOSviewer layout, clustering and display controls. | The screenshots are used only as evidence of visible controls; thresholds, seeds and hidden settings are not inferred from them. The manuscript and retained item tables remain authoritative for reported node statistics. |
...
| 5. Add fusion-level comparison | Addressed | Table *Design and evaluation considerations for multimodal LULC fusion levels* compares integration requirements, representative complexity, scalability, failure modes, evaluation evidence, and suitable conditions. Representative implementations are cited in the table note, while the risks and evaluation recommendations are identified as the authors' synthesis. | Cross-paper accuracy and runtime values were not pooled because datasets, resolutions, hardware, training budgets, and splits differ; presenting them as directly comparable would be misleading. |
...
| 10. Reproducibility and open science | Addressed | A dedicated section links to the public repository and discusses benchmarks, code/configuration, spatial and temporal hold-outs, seeds, preprocessing, compute, per-class/calibration metrics, and missing-modality tests. The repository includes the screened 261-record tagged-text input, matching UT manifest, thesauri, settings evidence, audit notes, and validation code. | Users remain responsible for the database-content reuse terms applicable to them. |
...
| 3. Missing fields, screening, cleaning, parameters, and 80.9% coding basis | Addressed | WoSCC interface/field/default coverage, query, filters, export, screening, duplicate rule, keyword cleaning, affiliation-name check, and VOSviewer version are reported. A tagged-text input and manifest identify all 261 records. Cropped panels document visible layout, clustering and display settings. The manuscript defines the 163-record fusion-level subset, and a record-level supplement documents those assignments. The percentage has been removed from the Abstract and Conclusion. | The screenshots establish only the controls they visibly contain. The six restored bibliometric records were not assigned fusion categories retrospectively, so the archived 163-record category totals remain unchanged. |
...
| 5. EuroSAT is not a multimodal fusion benchmark | Addressed | EuroSAT was removed from the representative multimodal benchmark table, MUUFL Gulfport was added, and Berlin/Augsburg modalities, dimensions, and resolutions were revised with source citations. EuroSAT remains visible only in the separate descriptive dataset-frequency figure and is not treated as a multimodal benchmark. | Dataset specifications should still be checked once more against the cited primary releases before submission. |

## Items that cannot be closed by wording alone

Two remaining production issues require source recovery or re-export rather than
additional wording:

1. reconciling institution-ranking differences between the retained CSV and the
   existing Figure 5/related prose;
2. regenerating the remaining raster line-art figures from verified source data
