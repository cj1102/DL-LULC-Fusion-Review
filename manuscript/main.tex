...
The technical evolution analysed in this review concerns heterogeneous multi-source fusion within the selected corpus. Feature-level fusion integrates complementary spectral, structural, and textural information from optical imagery, SAR, and LiDAR, with robustness and versatility depending on alignment, sample availability, computation, and application. The reviewed approaches include local feature extraction by CNNs and global context aware modelling enabled by Transformers and attention mechanisms. State-space models (e.g., Mamba) offer theoretically favourable sequence scaling, but their 2D multimodal and cross-region performance remains insufficiently validated.

Despite advances in model design, data reliability and model robustness remain critical bottlenecks. Spatiotemporal inconsistencies between multimodal sensors introduce ``pseudo-changes'' in fused observations. The ``black box'' nature of models limits their utility in decision-making where geographical interpretability is essential. Dependence on massive labelled datasets hinders generalisation to regions with scarce ground truth. Small benchmark datasets do not by themselves validate global or temporally dynamic mapping.

Looking forward, this study identifies three critical directions for future LULC research. First, integrating explainable artificial intelligence (XAI) into fusion models can quantify feature contributions and connect deep learning outputs with geographical process understanding. Second, remote sensing foundation models may support few-shot adaptation or zero-shot transfer when high-quality annotations are scarce, but their geographic bias, leakage risks, computational cost, and cross-region reliability require evaluation. Third, intelligent data governance and harmonisation require registration and normalisation algorithms to address sensor discrepancies and provide ``model-ready'' data while limiting spatiotemporal inconsistency. Priorities also include uncertainty calibration, domain adaptation and generalisation, continual learning, and standardised geographic and temporal evaluation protocols.
